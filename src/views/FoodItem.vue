      ...
      <div class="nutrientCon vitamins">
        <table>
          <tbody>
            ...
            <tr
              v-for="nutrient in macrosAndMicros.micros.vitamins"
              :key="nutrient.name"
            >
              <th>{{ nutrient.name }}</th>
...
export default {
  ...
  computed: {
    macrosAndMicros: function() {
      ...
      this.foodItem.foodNutrients.forEach(nutrient => {
        if (
          nutrient.nutrient.name.includes("Vitamin") &&
  ...
  methods: {
    findItem: function() {
      ...
      axios
        .get(`https://api.nal.usda.gov/fdc/v1/${fdcId}?${db_config.API_KEY}`)
        .then(response => {
          self.foodItem = response.data;

          // food category attribute title varies
          self.foodCategory =
            self.foodItem.foodCategory === undefined
              ? self.foodItem.brandedFoodCategory
              : self.foodItem.foodCategory;
        })
        .catch(err => {
          console.error(err);
        });
