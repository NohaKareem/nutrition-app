<template>
  <div id="app">
    ...
    <div
      class="results"
      v-on:gotoNextPage="nextPage()"
      v-on:gotoPrevPage="prevPage()"
    >
    ...
    <footer>
      Data by U.S. Department of Agriculture, Agricultural Research Service.
...
export default {
  ...
  computed: {
    isSearchPage: function() {
      return !this.routeName.includes("foodItem");
    },
    // show welcome message if no search string is input and page is a search page
    showWelcomeMessage: function() {
      return this.searchStr == "" && this.isSearchPage;
    },
    routeName() {
  ...
  methods: {
    ...
    foodSearch: function() {
      ...
      axios
        .get(
          `https://api.nal.usda.gov/fdc/v1/search?${db_config.API_KEY}&generalSearchInput=${this.searchStr}&pageNumber=${this.currentPage}`
        )
        .then(response => {
    ...
    hasNextPage: function() {
      let totalPages = Math.ceil(
        this.searchResults.totalHits / this.RESULTS_PER_PAGE
      );
